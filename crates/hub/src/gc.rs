...
pub async fn cleanup_idle_pods(
    ...
    for pod in pods.items {
        ...
        if let Some(annotations) = &pod.metadata.annotations {
            if let Some(expires_at_str) = annotations.get(TTL_ANNOTATION) {
                if let Ok(expires_at) = expires_at_str.parse::<u64>() {
                    if now > expires_at {
                        info!(
                            "GC: Pod {} has exceeded its max TTL. Deleting.",
                            pod_name
                        );
        ...
        let health_url =
            format!("http://{}.default.svc.cluster.local:8080/health", service_name);

        match client.get(&health_url).timeout(std::time::Duration::from_secs(5)).send().await {
            Ok(response) => {
                if !response.status().is_success() {
                    warn!("GC: Health check for {} failed (status: {}). Deleting.", pod_name, response.status());
                    pod_api.delete(pod_name, &DeleteParams::default()).await?;
                    continue;
                }
                ...
                match response.json::<SidecarHealth>().await {
                    Ok(health) => {
                        ...
                        if health.idle_seconds > max_idle_seconds { // <-- Use configured value
                            info!("GC: Pod {} exceeded idle time. Deleting.", pod_name);
                            pod_api.delete(pod_name, &DeleteParams::default()).await?;
                        }
                    }
                    Err(e) => {
                        warn!("GC: Failed to parse health from {}: {}. Deleting.", pod_name, e);
                        pod_api.delete(pod_name, &DeleteParams::default()).await?;
                    }
                }
            }
            Err(e) => {
                warn!("GC: Health check request for {} failed: {}. Deleting.", pod_name, e);
                pod_api.delete(pod_name, &DeleteParams::default()).await?;
            }
        }
    }
